...
@section('content')
<div class="space-y-6">
    <form class="ajax-form w-full space-y-6" id="myForm" action="{!! route('dashboard.roles.update', $role->id) !!}" method="POST" enctype="multipart/form-data" novalidate
        ...
        <div class="dash-card p-6 space-y-4">
            ...
            @if ($role->id === 1)
                ...
                <div class="p-4 rounded-2xl bg-amber-50/80 dark:bg-amber-950/40 border border-amber-200/80 dark:border-amber-900/50 flex items-start gap-3">
                    <i class="fas fa-shield-alt text-amber-600 dark:text-amber-400 mt-0.5 text-base"></i>
                    <div class="text-xs">
                        <span class="font-bold text-amber-900 dark:text-amber-200 block mb-0.5">{!! __('roles.system_role_protected') !!}</span>
                        <p class="text-slate-600 dark:text-slate-400 leading-relaxed">{!! __('roles.super_admin_protection_msg') !!}</p>
                    </div>
                </div>
            @endif
                ...
                <div>
                    ...
                    <select name="store_id" id="store_id" class="form-input-modern select2">
                        ...
                        @foreach ($stores as $store)
                            <option value="{{ $store->id }}" @selected($role->store_id == $store->id)>{{ $store->name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="name_ar" name="name[ar]" value="{!! old('name.ar', $role->getTranslation('name', 'ar')) !!}"
                        class="form-input-modern" placeholder="{!! __('roles.enter_role_ar') !!}" autocomplete="off" @disabled($role->id === 1)>
                ...
                <div>
                    ...
                    <input type="text" id="name_en" name="name[en]" value="{!! old('name.en', $role->getTranslation('name', 'en')) !!}"
                        class="form-input-modern" placeholder="{!! __('roles.enter_role_en') !!}" autocomplete="off" @disabled($role->id === 1)>
            ...
            <div>
                ...
                <input type="text" id="description" name="description" value="{!! old('description', $role->description) !!}"
                    class="form-input-modern" placeholder="{!! __('roles.enter_description') ?? 'ادخل وصفاً لهذا الدور...' !!}" autocomplete="off" @disabled($role->id === 1)>
        ...
        @php
            $rolePermissions = $role->permissions instanceof \Illuminate\Support\Collection 
                ? $role->permissions->pluck('name')->toArray() 
                : (is_array($role->permissions) ? $role->permissions : (json_decode($role->permissions, true) ?? []));
        @endphp
                    ...
                    @if(auth()->user()->id === 1 || auth()->user()->role_id === 1 || Gate::allows($moduleKey))
                        @php 
                            ...
                            foreach ($operations as $opKey => $opLangKey) {
                                $permName = $moduleKey . '_' . $opKey;
                                if (auth()->user()->id === 1 || auth()->user()->role_id === 1 || auth()->user()->hasAbility($permName)) {
                                    $visiblePerms[] = $permName;
                                }
                            }
                            $allModuleChecked = count($visiblePerms) > 0 && ($role->id === 1 || empty(array_diff($visiblePerms, $rolePermissions)));
                        @endphp
                        ...
                        <div class="dash-card overflow-hidden flex flex-col justify-between group hover:border-indigo-200 dark:hover:border-indigo-800 transition-all">
                            ...
                            <div class="p-4 bg-slate-50/80 dark:bg-slate-800/60 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                                ...
                                <label class="relative inline-flex items-center cursor-pointer select-none" title="تحديد الكل">
                                    <input type="checkbox" class="sr-only peer select-all-module" data-module="module-{{ $moduleKey }}" @checked($allModuleChecked) @disabled($role->id === 1 || $moduleKey === 'stores')>
                            ...
                            <div class="p-4 space-y-3 flex-1 divide-y divide-slate-100 dark:divide-slate-800/60">
                                @foreach ($operations as $opKey => $opLangKey)
                                    @php 
                                        $permName = $moduleKey . '_' . $opKey; 
                                        $isChecked = in_array($permName, $rolePermissions) || $role->id === 1;
                                    @endphp
                                    @if(auth()->user()->id === 1 || auth()->user()->role_id === 1 || auth()->user()->hasAbility($permName))
                                        <div class="flex items-center justify-between pt-3 first:pt-0">
                                            ...
                                            <label class="relative inline-flex items-center cursor-pointer select-none shrink-0">
                                                <input type="checkbox" class="sr-only peer permission-checkbox module-{{ $moduleKey }}" name="permissions[]" value="{{ $permName }}" @checked($isChecked) @disabled($role->id === 1 || $moduleKey === 'stores')>
