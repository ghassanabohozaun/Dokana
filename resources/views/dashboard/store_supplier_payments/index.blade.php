...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div>
                    <div class="flex items-center gap-2.5">
                        <h1 class="text-base sm:text-lg font-bold text-slate-800 dark:text-white">
                            {!! __('store_supplier_payments.store_supplier_payments_list') !!}
                        </h1>
                        <span id="total-count-badge" class="badge-pill badge-pill-info text-[11px]">
                            {!! $payments->total() !!} {!! __('general.records') !!}
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="flex items-center gap-2.5 self-start sm:self-center">
            @can('store_supplier_payments_create')
                <button type="button" class="btn-primary-gradient" data-toggle="modal" data-target="#createStoreSupplierPaymentModal">
                    <i class="fas fa-plus text-xs"></i>
                    <span>{!! __('store_supplier_payments.create_new_store_supplier_payment') !!}</span>
                </button>
            @endcan
        </div>
    </div>
    ...
    <!-- 3. Main Payments Data Table Card -->
