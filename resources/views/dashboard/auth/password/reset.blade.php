...
<div class="h-screen w-screen grid grid-cols-1 lg:grid-cols-12 overflow-hidden bg-slate-50 dark:bg-slate-950 relative selection:bg-indigo-500 selection:text-white"
    x-data="{ showPassword: false, showConfirmPassword: false, isSubmitting: false }">
    ...
    <div class="hidden lg:flex lg:col-span-6 xl:col-span-7 h-screen bg-gradient-to-br from-indigo-700 via-indigo-800 to-slate-950 dark:from-slate-950 dark:via-slate-900 dark:to-indigo-950 text-white p-10 xl:p-16 flex-col relative overflow-hidden select-none border-e border-white/10 dark:border-slate-800">
        ...
        <div class="flex-1 flex flex-col justify-center relative z-10 max-w-xl py-6">
            <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/10 border border-white/15 text-xs font-bold text-indigo-100 shadow-xs mb-5 w-fit">
                <i class="fas fa-lock-open text-amber-300"></i>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="p-4 rounded-2xl bg-white/[0.07] border border-white/15 hover:bg-white/[0.12] transition-colors">
                    ...
                    <h3 class="text-xs xl:text-sm font-bold text-white mb-1">{{ __('auth.reset_password') }}</h3>
                    <p class="text-[11px] text-indigo-100/75 leading-normal">{{ __('auth.your_password_updated_successfully') }}</p>
                </div>
        ...
        <div class="flex-1 flex flex-col justify-center w-full max-w-md mx-auto py-2 sm:py-6">
            ...
            <div class="snake-border-card shadow-2xl shadow-indigo-500/10 lg:shadow-none">
                ...
                <div class="snake-border-inner p-6 sm:p-8 bg-white/95 dark:bg-slate-900/95 lg:bg-transparent lg:dark:bg-transparent lg:p-0 border border-slate-200/80 dark:border-slate-800 lg:border-0 backdrop-blur-xl lg:backdrop-blur-none transition-all">
                    ...
                    <div class="flex lg:hidden items-center justify-between gap-2 pb-5 mb-5 border-b border-slate-100 dark:border-slate-800/80">
                        <div class="flex items-center gap-2">
                            ...
                            <div class="leading-none text-start">
                                ...
                                <span class="text-[9px] font-bold text-indigo-600 dark:text-indigo-400 tracking-wider">SECURITY</span>
                            </div>
                        </div>
                    ...
                    <!-- Form (Anti-Autofill Protected) -->
                    <form action="{!! route('dashboard.password.post.reset') !!}" method="POST" @submit="isSubmitting = true" autocomplete="off" class="space-y-4" novalidate>
                        @csrf
                        ...
                        <div>
                            <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">
                                {!! __('auth.you_email_address') !!}
                            </label>
                            <div class="relative">
                                <div class="absolute inset-y-0 start-0 flex items-center ps-3.5 pointer-events-none text-slate-400 dark:text-slate-500">
                                    <i class="fas fa-envelope text-xs"></i>
                                </div>
                                <input type="email" name="email" value="{{ $email }}" readonly autocomplete="off"
                                    class="w-full ps-10 pe-4 py-3 rounded-xl text-xs bg-slate-100 dark:bg-slate-800/90 border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 font-mono font-medium cursor-not-allowed select-none">
                            </div>
                        </div>
                        ...
                        <div>
                            <label for="password" class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">
                                {!! __('auth.enter_password') !!} <span class="text-rose-500">*</span>
                            </label>
                            <div class="relative group">
                                ...
                                <input type="password" :type="showPassword ? 'text' : 'password'" name="password" id="password" required autofocus autocomplete="new-password"
                        ...
                        <!-- Confirm Password -->
                        <div>
                            <label for="confirm_password" class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">
                                {!! __('auth.enter_confirm_password') !!} <span class="text-rose-500">*</span>
                            </label>
                            <div class="relative group">
                                <div class="absolute inset-y-0 start-0 flex items-center ps-3.5 pointer-events-none text-slate-400 group-focus-within:text-indigo-600 dark:group-focus-within:text-indigo-400 transition-colors">
                                    <i class="fas fa-check-double text-xs"></i>
                                </div>
                                <input type="password" :type="showConfirmPassword ? 'text' : 'password'" name="confirm_password" id="confirm_password" required autocomplete="new-password"
                                    readonly onfocus="this.removeAttribute('readonly');"
                                    class="w-full ps-10 pe-10 py-3 rounded-xl text-xs bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 font-medium cursor-text transition-all"
                                    placeholder="••••••••">
                                <button type="button" @click="showConfirmPassword = !showConfirmPassword"
                                    ...
                                    <i class="fas text-xs" :class="showConfirmPassword ? 'fa-eye-slash' : 'fa-eye'"></i>
                                </button>
                            </div>
                        </div>
                        ...
                        <button type="submit" :disabled="isSubmitting"
                            ...
                            <span x-text="isSubmitting ? '{{ __('general.loading') ?? 'جاري الحفظ...' }}' : '{{ __('auth.reset_password') }}'">{!! __('auth.reset_password') !!}</span>
                            <i class="fas fa-save text-xs" x-show="!isSubmitting"></i>
                        </button>
                    </form>
