        ...
        <div class="flex-1 flex flex-col justify-center relative z-10 max-w-xl py-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="p-4 rounded-2xl bg-white/[0.07] border border-white/15 hover:bg-white/[0.12] transition-colors">
                    ...
                    <h3 class="text-xs xl:text-sm font-bold text-white mb-1">{{ __('auth.verify_code') }}</h3>
                    <p class="text-[11px] text-indigo-100/75 leading-normal">{{ __('auth.enter_the_code_sent_to_your_email') }}</p>
                </div>
                ...
                <div class="snake-border-inner p-6 sm:p-8 bg-white/95 dark:bg-slate-900/95 lg:bg-transparent lg:dark:bg-transparent lg:p-0 border border-slate-200/80 dark:border-slate-800 lg:border-0 backdrop-blur-xl lg:backdrop-blur-none transition-all">
                    ...
                    <div class="flex lg:hidden items-center justify-between gap-2 pb-5 mb-5 border-b border-slate-100 dark:border-slate-800/80">
                        <div class="flex items-center gap-2">
                            ...
                            <div class="leading-none text-start">
                                ...
                                <span class="text-[9px] font-bold text-indigo-600 dark:text-indigo-400 tracking-wider">VERIFY</span>
                            </div>
                        </div>
                    ...
                    <div class="mb-5">
                        ...
                        <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
                            {!! __('auth.enter_the_code_sent_to_your_email') !!}
                        </p>
                    </div>
                    ...
                    <!-- Verify Form (Anti-Autofill Protected) -->
                    <form action="{!! route('dashboard.password.post.verify') !!}" method="POST" @submit="isSubmitting = true" autocomplete="off" class="space-y-4" novalidate>
                        @csrf
                        ...
                        <div>
                            <label for="code" class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">
                                {!! __('auth.verification_code') ?? 'رمز التحقق' !!} <span class="text-rose-500">*</span>
                            </label>
                            <div class="relative">
                                <input type="text" name="code" id="code" required autofocus maxlength="10" autocomplete="off"
                                    readonly onfocus="this.removeAttribute('readonly');"
                                    class="w-full text-center tracking-[0.5em] text-lg font-mono py-3.5 px-4 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 font-black cursor-text transition-all"
                                    placeholder="••••••">
                            </div>
                        </div>
                        ...
                        <button type="submit" :disabled="isSubmitting"
                            ...
                            <span x-text="isSubmitting ? '{{ __('general.loading') ?? 'جاري التحقق...' }}' : '{{ __('auth.verify') ?? 'تأكيد الرمز' }}'">{!! __('auth.verify') ?? 'تأكيد الرمز' !!}</span>
                            <i class="fas fa-check-circle text-xs" x-show="!isSubmitting"></i>
                        </button>
                    </form>
