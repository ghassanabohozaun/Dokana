...
<div x-data="{ show: false }" 
    ...
    <div x-show="show" 
        ...
        <div class="flex-1 overflow-y-auto relative p-5 custom-scrollbar">
            ...
            <div x-show="isSummaryLoading" class="absolute inset-0 z-10 flex flex-col items-center justify-center bg-gray-50/80 dark:bg-darkCard/80 backdrop-blur-sm" x-cloak>
                <i class="ph-bold ph-spinner-gap animate-spin text-4xl text-primary mb-3"></i>
                <p class="text-gray-500 font-bold text-sm">{{ __('notebook.loading_reports') ?? 'جاري جلب التقارير...' }}</p>
            </div>
            ...
            <template x-if="summaryData">
                <div class="space-y-6 pb-4">
                    ...
                    <div class="grid gap-4">
                        ...
                        <!-- Collections -->
                        <div class="bg-white dark:bg-darkCard rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-800 flex items-center justify-between hover:shadow-md transition-shadow">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-sm text-gray-500 dark:text-gray-400 font-bold">{{ __('notebook.collections') ?? 'التحصيلات (دفعات)' }}</p>
                                    <p class="text-xs text-emerald-500 font-bold mt-0.5">{{ __('notebook.collections_desc') ?? 'أموال دخلت الصندوق' }}</p>
                                </div>
                            </div>
                        ...
                        <div class="bg-white dark:bg-darkCard rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-800 flex items-center justify-between hover:shadow-md transition-shadow">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-sm text-gray-500 dark:text-gray-400 font-bold">{{ __('notebook.direct_sales_summary') ?? 'المبيعات الفورية' }}</p>
                                    <p class="text-xs text-blue-500 font-bold mt-0.5">{{ __('notebook.direct_sales_desc') ?? 'كاش مباشر' }}</p>
                                </div>
                            </div>
                            <div class="text-end shrink-0 pl-2">
                        ...
                        <div class="bg-white dark:bg-darkCard rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-800 flex items-center justify-between hover:shadow-md transition-shadow">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="text-sm text-gray-500 dark:text-gray-400 font-bold">{{ __('notebook.new_debts') ?? 'ديون جديدة' }}</p>
                                    <p class="text-xs text-red-500 font-bold mt-0.5">{{ __('notebook.new_debts_desc') ?? 'مبيعات بالآجل' }}</p>
                                </div>
                            </div>
                            <div class="text-end shrink-0 pl-2">
                    ...
                    <!-- Total Cash Summary -->
                    <div class="bg-gradient-to-br from-primary to-indigo-600 rounded-2xl p-5 text-white shadow-lg shadow-primary/30 relative overflow-hidden mt-4 hover:scale-[1.02] transition-transform">
