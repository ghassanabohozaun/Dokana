                ...
                <div class="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50/50 dark:bg-[#0b1121] custom-scrollbar relative">
                                ...
                                <div class="bg-white dark:bg-darkCard p-4 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm flex flex-col gap-3 mb-3 hover:shadow-md transition-shadow">
                                    <div class="flex justify-between items-center">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <!-- Customer Name -->
                                                <p @click="if(tx.customer) { activeCustomer = tx.customer; openLedger(tx.customer.id); $dispatch('close-modal', {id: 'todayCollectionsModal'}); }" 
                                                   ...
                                                   :class="tx.customer ? 'cursor-pointer hover:text-primary transition-colors' : ''">
                                                    ...
                                                    <template x-if="tx.customer && tx.customer.is_walk_in">
                                                        <span class="text-[9px] bg-primary/10 text-primary px-1.5 py-0.5 rounded-md mx-1">{{ __('notebook.walk_in_customer') ?? 'مبيعات فورية' }}</span>
                                                    </template>
                                                </p>
                                        ...
                                        <div class="text-left font-black shrink-0 text-xl text-emerald-500 flex flex-col items-end">
                                            ...
                                            <template x-if="tx.type === 'direct_sale'">
                                                <span class="text-[10px] bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 px-2 py-0.5 rounded-full mt-1">{{ __('notebook.direct_sale') ?? 'شراء فوري' }}</span>
                                            </template>
                                        </div>
                                    </div>
                    ...
                    <template x-if="totalTodayCollectionsCount > todayCollectionsList.length">
                        <div class="mt-6 flex justify-center pb-4">
                            <button @click="loadMoreCollections" :disabled="isCollectionsLoading" class="group relative px-6 py-3 text-xs font-bold text-gray-600 bg-white hover:bg-gray-100 shadow-sm border border-gray-200 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 dark:text-gray-300 rounded-full transition-all duration-75 flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed">
