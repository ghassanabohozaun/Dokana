...
<div x-data="{ show: false }" 
     x-show="show" 
     x-on:open-modal.window="if ($event.detail.id === 'addSupplierModal') { show = true; $nextTick(() => { $el.scrollTop = 0; }); }"
     x-on:close-modal.window="if ($event.detail.id === 'addSupplierModal') show = false"
     style="display: none;"
    ...
    <div class="w-full md:max-w-3xl min-h-screen flex flex-col bg-gray-50 dark:bg-[#0b1121] shadow-2xl relative">
        <div class="flex flex-col h-screen">
            ...
            <div class="flex-1 overflow-y-auto p-4 md:p-6 bg-white dark:bg-[#0b1121] custom-scrollbar relative">
                <form @submit.prevent="saveSupplier()" novalidate class="space-y-4">
                    <div>
                        ...
                        <input x-model="newSupplierName" type="text" required placeholder="{{ __('notebook.enter_supplier_name') }}" class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3.5 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 outline-none transition-all font-medium text-gray-900 dark:text-white placeholder-gray-400">
                    </div>
                    <div>
                        ...
                        <input x-model="newSupplierPhone" 
                               @input="newSupplierPhone = normalizeArabicNumbers(newSupplierPhone)" 
                               type="tel" 
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input x-model="newSupplierBankName" type="text" required placeholder="{{ __('notebook.enter_bank_name') }}" class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3.5 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 outline-none transition-all font-medium text-gray-900 dark:text-white placeholder-gray-400">
                        </div>
                        <div>
                            <label class="block text-sm font-bold mb-1.5 text-gray-700 dark:text-gray-300">{{ __('notebook.supplier_account_number') }}</label>
                            <input x-model="newSupplierAccountNumber" 
                                   @input="newSupplierAccountNumber = normalizeArabicNumbers(newSupplierAccountNumber)" 
                                   type="text" 
                    ...
                    <div>
                        ...
                        <input x-model="newSupplierAddress" type="text" placeholder="{{ __('notebook.supplier_address_placeholder') }}" class="w-full bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-3.5 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 outline-none transition-all font-medium text-gray-900 dark:text-white placeholder-gray-400">
                    </div>
            ...
            <div class="p-4 md:p-6 border-t dark:border-gray-800 bg-white dark:bg-darkCard shrink-0 sticky bottom-0 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                <button type="button" 
                    ...
                    <span x-show="!isSavingSupplier">{{ __('notebook.save') }}</span>
