    <!-- Today Direct Sales Overlay -->
    <div x-data="{ show: false }" 
         x-show="show" 
         x-on:open-modal.window="if ($event.detail.id === 'todayDirectSalesModal') { show = true; $nextTick(() => { if($refs.todayDirectSalesScroll) $refs.todayDirectSalesScroll.scrollTop = 0; $el.scrollTop = 0; }); }"
        ...
        <div class="w-full md:max-w-3xl min-h-screen flex flex-col bg-gray-50 dark:bg-[#0b1121] shadow-2xl relative">
            <div class="flex flex-col h-screen">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h2 class="font-bold text-lg text-gray-900 dark:text-white flex flex-wrap items-center gap-2">
                                <span>{{ __('notebook.direct_sales') ?? 'مبيعات فورية (اليوم)' }}</span>
                                <span class="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 border border-blue-200 dark:border-blue-800">
                                    <span x-text="totalTodayDirectSalesCount"></span> {{ __('notebook.transactions_count') ?? 'عملية' }}
                                </span>
                            </h2>
                            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5 font-medium flex items-center gap-1">
                                ...
                                <span>{{ __('notebook.direct_sales_desc') ?? 'سجل عمليات الشراء الفوري المسجلة اليوم' }}</span>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <!-- Transaction List -->
                <div x-ref="todayDirectSalesScroll" class="flex-1 overflow-y-auto p-4 pb-28 space-y-3 bg-gray-50/50 dark:bg-[#0b1121] custom-scrollbar relative">
                    <div x-show="isDirectSalesLoading && todayDirectSalesList.length === 0" class="absolute inset-0 bg-white/50 dark:bg-black/50 z-10 flex items-center justify-center backdrop-blur-sm" x-cloak>
                        <i class="ph-bold ph-spinner-gap animate-spin text-4xl text-primary"></i>
                    </div>

                    <template x-if="todayDirectSalesList.length === 0 && !isDirectSalesLoading">
                                    ...
                                    <div class="flex justify-between items-start">
                                        <div class="flex items-center gap-3">
                                            <div class="w-10 h-10 rounded-full flex items-center justify-center shrink-0 bg-blue-50 text-blue-500 dark:bg-blue-900/20">
                                                <i class="ph-bold text-lg ph-shopping-cart-simple"></i>
                                            </div>
                                            <div>
                                                <!-- Customer Name -->
                    ...
                    <template x-if="totalTodayDirectSalesCount > todayDirectSalesList.length">
                        <div class="mt-6 mb-10 flex justify-center pb-14 px-2">
                            <button @click="loadMoreDirectSales" :disabled="isDirectSalesLoading" 
                                ...
                                <span class="flex items-center gap-2" x-show="!isDirectSalesLoading">
                                    <span>{{ __('notebook.show_older_transactions') ?? 'عرض المزيد من العمليات' }}</span>
                                    <i class="ph-bold ph-caret-down text-base"></i>
                                </span>
                                <span class="flex items-center gap-2" x-show="isDirectSalesLoading" style="display: none;">
