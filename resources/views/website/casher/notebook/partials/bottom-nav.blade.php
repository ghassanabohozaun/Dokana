    ...
    <div class="w-full bg-white/85 dark:bg-[#0f172a]/85 backdrop-blur-xl border-t sm:border border-white/40 dark:border-gray-700/50 shadow-[0_-8px_30px_rgb(0,0,0,0.12)] rounded-t-[1.5rem] sm:rounded-[2rem] px-2 py-2 sm:py-3 flex justify-between items-center relative pointer-events-auto overflow-hidden transition-all duration-75">
        ...
        <div class="flex flex-1 justify-around relative z-10">
            ...
            <button @click="switchTab('customers')"
                class="flex flex-col items-center justify-center py-1.5 transition-all duration-75 w-16 group relative"
                :class="activeTab === 'customers' ? 'text-cyan-500' : 'text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300'">
                <div class="relative transition-all duration-75 transform group-active:scale-95"
                ...
                <!-- Active Dot -->
                <div class="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-cyan-500 transition-all duration-75" :class="activeTab === 'customers' ? 'opacity-100 scale-100' : 'opacity-0 scale-0'"></div>
            </button>
            ...
            <button @click="switchTab('withdrawals')"
                ...
                <!-- Active Dot -->
                <div class="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-rose-500 transition-all duration-75" :class="activeTab === 'withdrawals' ? 'opacity-100 scale-100' : 'opacity-0 scale-0'"></div>
            </button>
        </div>
        ...
        <div class="flex flex-1 justify-around relative z-10">
            ...
            <div class="flex flex-col items-center justify-center py-1.5 text-gray-500 dark:text-gray-400 w-16 group cursor-default">
                <div class="relative transition-all duration-75 transform group-hover:-translate-y-1">
            ...
            <a href="{{ route('website.casher.logout') }}" 
               class="flex flex-col items-center justify-center py-1.5 transition-all duration-75 text-gray-400 hover:text-red-500 w-16 group relative"
               title="{{ __('notebook.logout') ?? 'خروج' }}">
                <div class="relative transition-all duration-75 transform group-hover:-translate-y-1 group-active:scale-95">
